...
/* --- GT911 reset (sélection adresse 0x5D) --- */
...
/* --- LVGL display glue (LovyanGFX -> LVGL) --- */
...
static void lv_flush_cb(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
  ...
  // LV_COLOR_DEPTH=16 => px_map est un buffer RGB565
...
/* --- UI: jauge à aiguille + bouton moteur --- */
static lv_obj_t * scale_obj;
static lv_obj_t * needle_img;
static lv_obj_t * temp_label;
static lv_obj_t * motor_btn;
static lv_obj_t * motor_btn_label;
...
{
  // Adapte ici selon ton driver moteur
...
static void update_needle(int32_t value_celsius)
{
  ...
  // 0°C = -135° (bas gauche), 250°C = 0° (haut), 500°C = +135° (bas droite)
  ...
  // Optionnel si tu veux forcer un refresh immédiat
  // lv_obj_invalidate(needle_img);
}
...
static void motor_btn_event_cb(lv_event_t * e)
{
  ...
  if(motor_on) {
    lv_obj_set_style_bg_color(motor_btn, lv_palette_main(LV_PALETTE_GREEN), 0);
  } else {
...
static void create_ui()
{
  ...
  lv_obj_align(scale_obj, LV_ALIGN_TOP_MID, 0, 50);
  ...
  lv_scale_set_total_tick_count(scale_obj, 51);
  ...
  // --- Aiguille : parent transparent de la taille du cadran ---
  ...
  lv_obj_add_flag(needle_parent, LV_OBJ_FLAG_OVERFLOW_VISIBLE); // IMPORTANT: ne pas couper l’aiguille
  ...
  const int needle_h = 90; // longueur aiguille (ajuste si tu veux)
  ...
  // Place l’aiguille pour que sa BASE soit au centre du cadran
  // centre du parent = (120,120) car parent 240x240
  lv_obj_set_pos(needle_img, 120 - needle_w/2, 120 - needle_h);

  // Pivot rotation = bas-centre de l’aiguille
  ...
  lv_style_set_text_font(&temp_style, &lv_font_montserrat_24);   // taille de police
...
/* Variables pour simulation température */
static int32_t current_temp = 0;
static int32_t temp_delta = 1;
...
static void lvgl_setup()
{
  ...
  static lv_color_t buf1[320 * BUF_LINES];
...
{
  lv_timer_handler();
  // Simulation température : incrémente de 1 toutes les 200ms
  ...
  if(now - last_update >= 200) {
    ...
    if(current_temp >= 500 || current_temp <= 0) {
